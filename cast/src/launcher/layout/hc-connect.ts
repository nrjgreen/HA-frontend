...
const translateErr = (err) =>
  err === ERR_CANNOT_CONNECT
    ...
    : err === ERR_HASS_HOST_REQUIRED
      ? "Please enter a NRJHub URL."
      : err === ERR_INVALID_HTTPS_TO_HTTP
        ? html`
            Cannot connect to NRJHub instances over "http://".
            ${seeFAQ("https")}
          `
        : `Unknown error (${err}).`;
...
const INTRO = html`
  <p>
    NRJHub Cast allows you to cast your NRJHub installation to
...
export class HcConnect extends LitElement {
      ...
      return html`
        <hc-layout>
          <div class="card-content">
            ${INTRO}
            <p>
              To get started, enter your NRJHub URL and click authorize.
  ...
  private async _handleConnect() {
    ...
    if (value === "") {
      this.error = "Please enter a NRJHub URL.";
      return;
    }
    if (value.indexOf("://") === -1) {
      this.error =
