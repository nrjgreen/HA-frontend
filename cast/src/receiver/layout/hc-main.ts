...
export class HcMain extends HassElement {
  ...
  private async _handleGetStatusMessage(msg: GetStatusMessage) {
    if (
      ...
      this._error = "Not connected to the same NRJHub instance.";
      this._sendError(
  ...
  private async _handleShowLovelaceMessage(msg: ShowLovelaceViewMessage) {
    ...
    if (
      ...
      this._sendStatus(msg.senderId!);
      this._error =
        "Cannot show Lovelace because we're not connected to the same NRJHub instance.";
      this._sendError(
        ReceiverErrorCode.WRONG_INSTANCE,
        this._error,
        msg.senderId!
  ...
  private _getErrorMessage(error: number): string {
    switch (error) {
      case 1:
        return "Unable to connect to the NRJHub websocket API.";
      case 2:
        return "The supplied authentication is invalid.";
      case 3:
        return "The connection to NRJHub was lost.";
      case 4:
        return "Missing hassUrl. This is required.";
      case 5:
        return "NRJHub needs to be served over https:// to use with NRJHub Cast.";
      default:
